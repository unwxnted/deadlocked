...
impl Player {
    ...
    pub fn bone_position(&self, cs2: &CS2, bone_index: usize) -> Vec3 {
        let gs_node = self.game_scene_node(cs2);
    ...
    pub fn visible(&self, cs2: &CS2, local_player: &Player) -> bool {
        if let Some(bvh) = &cs2.bvh {
            ...
            if !CHECKED_BONES
                .iter()
                .any(|bone| bvh.has_line_of_sight(eye_pos, self.bone_position(cs2, bone.index())))
            {
                return false;
            }
        } else {
